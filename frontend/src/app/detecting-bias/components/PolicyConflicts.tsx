...
export function PolicyConflicts({ conflicts = [] }: PolicyConflictsProps) {
    return (
        <div className="bg-[#050505]/80 backdrop-blur-xl border border-white/5 rounded-[2.5rem] p-8 space-y-6">
            ...
            <div className="space-y-4">
                {conflicts.length > 0 ? conflicts.map((p, i) => (
                    <div key={i} className="p-5 bg-white/[0.02] border border-white/5 rounded-2xl space-y-2">
                        ...
                        <p className="text-[11px] font-medium text-white/40 italic leading-relaxed">{p.description}</p>
                    </div>
                )) : (
                    <p className="text-xs text-white/30 italic">No policy conflicts detected.</p>
                )}
            </div>
        </div>
    );
